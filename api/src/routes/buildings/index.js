...
async function default_1(fastify) {
    fastify.get('/', {
        ...
        const blockWhere = {};
        const unitWhere = { status: { not: 'Sold' } };
        if (query.search) {
            where.OR = [
        ...
        if (query.area?.length)
            where.area = { in: query.area };
        if (query.has_pool)
            where.hasPool = true;
        if (query.has_view)
            where.hasView = true;
        if (query.category)
            blockWhere.category = { equals: query.category, mode: 'insensitive' };
        if (query.ownership)
            blockWhere.typeOfOwnership = query.ownership;
        if (query.price_min)
            unitWhere.price = { ...unitWhere.price, gte: query.price_min };
        if (query.price_max)
            unitWhere.price = { ...unitWhere.price, lte: query.price_max };
        if (query.area_m2_min)
            unitWhere.areaM2 = { ...unitWhere.areaM2, gte: query.area_m2_min };
        if (query.area_m2_max)
            unitWhere.areaM2 = { ...unitWhere.areaM2, lte: query.area_m2_max };
        if (query.rooms?.length)
            unitWhere.rooms = { in: query.rooms };
        ...
        const result = {
            items: items.map(buildings_service_1.transformBuildingToWpFormat),
            total,
            page: query.page,
    ...
    fastify.get('/:id', async (request, reply) => {
        ...
        const result = (0, buildings_service_1.transformBuildingToWpFormat)(building);
        await fastify.redis.set(cacheKey, JSON.stringify(result), 'EX', 300);
        return result;
    });
    // POST /api/buildings
