...
async function processChat(prisma, message, context, conversationHistory) {
    ...
    try {
        ...
        if (responseMessage.tool_calls && responseMessage.tool_calls.length > 0) {
            ...
            const whereBuilding = {};
            const whereBlock = {};
            const whereUnit = { status: { not: 'Sold' } }; // Only available units
            ...
            if (filters.hasPool !== undefined)
                whereBuilding.hasPool = filters.hasPool;
            if (filters.hasCarAccess !== undefined)
                whereBuilding.hasCarAccess = filters.hasCarAccess;
            if (filters.hasParking !== undefined)
                whereBuilding.hasParking = filters.hasParking;
            if (filters.category)
                whereBlock.category = { equals: filters.category, mode: 'insensitive' };
            if (filters.completionStage)
                whereBlock.constructionStage = { equals: filters.completionStage, mode: 'insensitive' };
            if (filters.ownershipType)
                whereBlock.typeOfOwnership = { equals: filters.ownershipType, mode: 'insensitive' };
            if (filters.minPrice)
                whereUnit.price = { ...whereUnit.price, gte: filters.minPrice };
            if (filters.maxPrice)
                whereUnit.price = { ...whereUnit.price, lte: filters.maxPrice };
            if (filters.minArea)
                whereUnit.areaM2 = { ...whereUnit.areaM2, gte: filters.minArea };
            if (filters.maxArea)
                whereUnit.areaM2 = { ...whereUnit.areaM2, lte: filters.maxArea };
            if (filters.minBedrooms)
                whereUnit.rooms = { ...whereUnit.rooms, gte: filters.minBedrooms };
            if (filters.maxBedrooms)
                whereUnit.rooms = { ...whereUnit.rooms, lte: filters.maxBedrooms };
            if (filters.propertyType)
                whereUnit.apartmentTypes = { has: filters.propertyType };
